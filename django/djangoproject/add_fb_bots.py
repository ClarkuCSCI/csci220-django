...
def main():
    ...
    argparser.add_argument(
        ...
        type=int,
        required=True,
        help="Number of profiles to add",
    )
    argparser.add_argument(
        "--max-statuses",
        type=int,
    ...
    with transaction.atomic():
        new_profiles = [random_profile() for _ in range(args.new_profiles)]
        ...
        for p in new_profiles:
            ...
            for _ in range(random.randint(1, args.max_statuses)):
                ...
                new_statuses.append(
                    Status(profile=p, message=message, date_time=timezone.now())
                )
            ...
            for pokee_id in random.sample(
                ...
                new_pokes.append(
                    Poke(poker=p, pokee_id=pokee_id, date_time=timezone.now())
                )
        Status.objects.bulk_create(new_statuses)
        Poke.objects.bulk_create(new_pokes)
    print(f"Added {args.new_profiles} fake profiles")
